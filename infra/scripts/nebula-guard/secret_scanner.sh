...
patterns=(
    ...
    'BOT_TOKEN[:=].+[0-9]{9,}:[a-zA-Z0-9_-]{35,}'        # Telegram Bot Token
    'DATABASE_URL[:=].+mysql:|postgresql:|sqlite:'       # Database URLs
)
...
for file in $files; do
    # Пропускаем сам сканер и системные папки, чтобы избежать ложных срабатываний
    if [[ "$file" == "infra/scripts/nebula-guard/secret_scanner.sh" ]] || [[ "$file" == .agent/* ]] || [[ "$file" == .vscode/* ]] || [[ "$file" == .gemini/* ]]; then
        continue
    fi
